function [nav_ima_phase_unwrapped_diff, fitted_nav_ima_phase, linear_phase_xy,global_phase, global_phase_diff_initial] = ...
    rigidMotion_parameter_calculation(nav_im_to_process, nav_ima_phase_unwrapped, ref_shot_idx, valid_data_ratio)

%INPUT
% 
%nav_im_to_process:         cpx 2D navigator in image space in size of [kx ky ch shot]
%nav_ima_phase_unwrapped:   unwrapped phase maps for nav_im_to_process
%ref_shot_idx:              reference shot #
%valid_data_ratio:          a portion of pixels with highest intensity are used for processing
% 
% 
% 
%OUTPUT
%
% nav_ima_phase_unwrapped_diff: unwrapped measured phase difference
% fitted_nav_ima_phase:         fitted phase difference
% linear_phase_xy:              linear phase coeffcient in rad per navigator pixel, in size of [2,ch,shot]
% global_phase:                 in rad, in size of [ch,shot]
% global_phase_diff_initial:    innitial guess of global phase difference from k space center

nav_kspa_to_process = bart('fft 3',nav_im_to_process);

[kx, ky, ch, shot] = size(nav_kspa_to_process)

assert(ref_shot_idx <= shot, 'reference shot is beyond total shot number');
assert((valid_data_ratio <= 1&valid_data_ratio >0), 'valid_data_ratio should between 0 and 1');

for ch_idx = 1:ch
    
    figure(3); montage(abs(nav_kspa_to_process(:,:,ch_idx,:)),'displayrange',[])
    figure(4); montage(angle(nav_kspa_to_process(:,:,ch_idx,:)),'displayrange',[-pi pi]); colormap jet
    ref_nav = nav_kspa_to_process(:,:,ch_idx,ref_shot_idx); %ref_shot_idx shot as the reference navigator
    
    for shot_idx = 1:shot
        
        current_nav = nav_kspa_to_process(:,:,ch_idx,shot_idx);
    ...
    for shot_idx = 1:shot
        nav_ima_raw_phase_diff(:,:,ch_idx,shot_idx) = angle(nav_im_to_process(:,:,ch_idx,shot_idx)) - angle(nav_im_to_process(:,:,ch_idx,ref_shot_idx));
    end
    figure(5); montage(nav_ima_raw_phase_diff(:,:,ch_idx,:),'displayrange',[-pi pi]); colormap jet; title('true measured phase difference')
    ...
    figure(6); montage(nav_ima_phase_unwrapped_diff(:,:,ch_idx,:),'displayrange',[-pi pi]); colormap jet; title('unwrapped measured phase difference')
    ...
    figure(7); montage(nav_ima_global_calibrated_phase_diff(:,:,ch_idx,:),'displayrange',[-pi pi]); colormap jet; title('phase difference after k = 0 phase correction')
    ...
    for shot_idx = 1:shot
        ...
        all_intensity = sort(col(abs(nav_im_to_process(:,:,ch_idx,shot_idx))));
        
        % pixels with high magnitude; not NaN; not Inf; being used for fitting
        image_intensity_cutoff = all_intensity(round(length(all_intensity)*(1-valid_data_ratio))); %0.5: 50% of the pixels were used; 0.8: 20% of the pixels were used
        
        idx_id = find((abs(nav_im_to_process(:,:,ch_idx,shot_idx))>image_intensity_cutoff)&(~isnan(fitting_nav_ima_phase_unwrapped))&(~isinf(fitting_nav_ima_phase_unwrapped)));
        ...
        title('To-be-fitted & fitted');
        subplot(122);imshow(fitting_results_2d,[ -5*pi 5*pi]); colormap jet
        title(['shot: ', num2str(shot_idx), 'ch: ', num2str(ch_idx)]);
        drawnow();
    ...
    figure(7);
    subplot(121);
    montage(nav_ima_phase_unwrapped_diff(:,:,ch_idx,:),'displayrange',[-5*pi 5*pi]); colormap jet; title('unwrapped measured phase difference')
    subplot(122);
    montage(fitted_nav_ima_phase(:,:,ch_idx,:),'displayrange',[-5*pi 5*pi]); colormap jet; title('fitted phase difference')
