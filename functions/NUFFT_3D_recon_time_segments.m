% OUTPUT
%
% nav_im_recon_nufft: reconed navigator image in size of [x,y,z,shot]
%
% 
% INPUT
% 
% nav_k_spa_data:   in size of [kx, ch, shots, <dyn>]
% trj_fn
% recon_par:
% b0_maps:          in size of [map_x, map_y, map_z] in unit of Hz
% nav_sense_map:    in size of [x, y, z, c]
% varargin{1}:      channel noise correlation matrix 
% varargin{2:3}:    FOV and offset in x and y
%
...
function nav_im_recon_nufft = NUFFT_3D_recon_time_segments(nav_k_spa_data,trj_fn,recon_par, b0_maps, nav_sense_map, varargin)
...
elseif(nargin >6 )
    assert(nargin == 8, 'Input both Offcenter and FOV in mm');
...
%% Resize b0 map same as sense map: Assuming B0 map have the same FOV
dim_sense = size(nav_sense_map);
dim_map = size(b0_maps);

[X, Y, Z] = meshgrid(linspace(1,dim_sense(1),dim_map(1)),linspace(1,dim_sense(2),dim_map(2)),linspace(1,dim_sense(3),dim_map(3)) );  %corrdinate for original locations
[Xq,Yq,Zq] = meshgrid(1:dim_sense(1),1:dim_sense(2),1:dim_sense(3) );  %corrdinate for intoplated locations
b0_maps_resized = interp3(X,Y,Z,b0_maps,Xq,Yq,Zq);
b0_maps_resized = b0_maps_resized.*(abs(nav_sense_map(:,:,:,1))>0);


figure(21); 
subplot(121); montage(permute(squeeze(abs(b0_maps)),[1 2 4 3]),'displayrange',[-200 200]); colormap jet; colorbar; xlabel('original B0 map(Hz)')
subplot(122); montage(permute(squeeze(abs(b0_maps_resized)),[1 2 4 3]),'displayrange',[-200 200]); colormap jet; colorbar; xlabel('resized B0 map(Hz)')
%% recon
...
for shot_nr = 1: end_shot_idx
    disp(['Current recon. shot: ',num2str(shot_nr)]);
    ...
    if(~recon_par.channel_by_channel) %(recon_par.sense_map_recon) %all in one recon
        ...
        nuFTop=nuFTOperator(trj_nufft,recon_par.recon_dim,double(nav_sense_map),6);
        
        A=tSegNUFTOperator(b0_maps_resized, recon_par.time_segments, selected_point, nuFTop, recon_par.recon_dim, size(nav_sense_map, 4));
