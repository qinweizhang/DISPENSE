...
nav_sens_map = nav_sens_map./max(abs(nav_sens_map(:)));
...
try
    if(recon_par.sense_map_recon) %all in one recon
        figure(37); montage(permute(squeeze(abs(nav_im_recon_nufft)), [1 2 4 3]),'displayrange',[]); title('all slices rss'); xlabel('P'); ylabel('M')
        figure(38); montage(permute(squeeze(abs(nav_im_recon_nufft)), [2 3 4 1]),'displayrange',[]); title('all slices rss'); xlabel('S'); ylabel('P')
    else
