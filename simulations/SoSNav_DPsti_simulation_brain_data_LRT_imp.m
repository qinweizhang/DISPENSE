...
if(ispc)
    cd('L:\basic\divi\Users\qzhang\SoSND\SoSND\simulations\brain_test_data')
else
...
%Get seperate navigator image
LRT_nav_mask_enable = false;

%Default TSE radial low-high acq.
load_default = false;     
%Or (predefined) sparse sampling pattern
...
%choose coil-idx [1-35]
ch_id = [3 5  6 10 15 20];

%choose shot-idx [1-42]
recon_shot_range = 1:42;

%%%%%%----------------------------%%%%%%%

%%%%-----simulate on a 2D ky-kz plane-----%%%%
...
for x = 1:length(recon_x_range)
    ...
    pars.method='LRT_implicit'; %POCS_ICE CG_SENSE_I CG_SENSE_K LRT LRT_implicit
...
%         LRT_nav_mask = ones(size(mask));
    ...
    if(strcmp( pars.method, 'LRT')) % for LRT separate navigator
        ...
        %prepare kspa to feed LRT package
        if(~isempty(LRT_nav_mask))
        %cat navigator and "b0" to feed LRT package
        ...
        %repeat kspa twice
            ...
            %remove the first colomn (b0) in the LRT.imref
            pars.LRT.Imref(:,:,1,:) = [];
        end
