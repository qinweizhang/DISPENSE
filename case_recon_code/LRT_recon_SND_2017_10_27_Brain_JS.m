...
b0_ima = bart('pics -RT:7:0:0.05 -d5', b0_kspa, TSE_sense_map);
...
for idx1 =1:length(pars.nonb0_shots)
    for idx2=1:2
        if(idx2==1) %nav colume
            ...
            kspa{idx1,idx2} =  nonb0_kpa(:,:,:,idx1).*250;
...
figure(678); imshow(abs(mean(image_corrected(:,:,:,:,2),4)),[])
