...
save traj_Sc4-5.mat trj_meas_kx trj_meas_ky trj_meas_kz
%% Recon with measured Trajecotry 
clear
fn = 'dp_25052017_1248364_4_2_wipdpstitseexperiment1senseV4.raw';
...
save('data_Sc4_2D.mat', 'nav_k_spa_data', '-append')


%% LOAD DATA
clear; close all;  load('traj_Sc4_5_knee.mat'); load('data_Sc4_2D.mat')

%% -----BART recon -------%
%scle trajectory to match kx_range, ky_range (-8 to 8)
close all;
ignor_kz = true;  % 1for yes. 0 for no
...
skip_point =0;
end_point = 1000; %length(trj_meas_kx);
...
scale_foctor_kz = 4/max(abs(trj_meas_kz));
...
subplot(122);  plot(abs(nav_k_spa_data(selected_point,:,nsa_nr,shot_nr,diffusion_nr))); xlabel('points'); ylabel('magnitude signal')
...
clear; close all; clc;
fn = 'data_Sc5_2D.mat';
 load('traj_Sc4_5_knee.mat'); load(fn);
...
%% NUFFT recon.  3D
clear; close all; clc;
load('traj_Sc3-4-5.mat'); load('data_Sc13_3D_Kz.mat')
%scle trajectory to match kx_range, ky_range (-pi to pi)
nsa_nr = 1;
shot_nr = 1; 
diffusion_nr =1;
...
trj_meas_ky_t = squeeze(trj_meas_ky(selected_point,1));
trj_meas_kz_t = squeeze(trj_meas_kz(selected_point,1));
sig_kspa = nav_k_spa_data(selected_point,:,nsa_nr,shot_nr,diffusion_nr);
...
for ch =1:14
    sig_nufft = double(sig_kspa(:, ch));
    sig_nufft = sig_nufft';
    A=nuFTOperator(trj_nufft,[16, 16, 13],ones(16, 16, 13),6);
...
%     im_recon_nufft(:,:,:,ch) = A'*sig_nufft';
     ...
     %call CG-SENSE with L2-norm regularization
     nav_im_recon_nufft(:,:,:,ch)=regularizedReconstruction(A,sig_nufft',@L2Norm,0.5,'maxit',25);
%     im_recon_nufft(:,:,:,ch)=regularizedReconstruction(A,sig_nufft','maxit',25);
end
% im_recon_nufft = flipdim(flipdim(im_recon_nufft,1),2);
figure(35); montage(permute(abs(nav_im_recon_nufft(:,:,:,6)), [1 2 4 3]),'displayrange',[]); title('all slices ch6')
figure(36); montage(-1* permute(angle(nav_im_recon_nufft(:,:,:,6)), [1 2 4 3]),'displayrange',[-pi pi]); colormap jet; title('all slices ch6')

nav_im_recon_nufft_rss = bart('rss 8', nav_im_recon_nufft);
