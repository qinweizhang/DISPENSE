...
clear; clc; close all
cd('/home/qzhang/lood_storage/divi/Ima/parrec/Kerry/Data/2017_10_04_SND_phantom')
...
data_fn = 'sn_03102017_2118302_25_2_wip_dpsti_sosad_linearV4.raw';
sense_ref_fn = 'sn_03102017_1953297_1000_13_wip_senserefscanV4.raw';
coil_survey_fn  = 'sn_03102017_1903472_1000_5_wip_coilsurveyscanV4.raw';
...
for dyn = 1:dyn_nr
    ...
    recon_par.acq_dim = [16 16 12];  
    recon_par.recon_dim  = [16 16 12];
...
nav_im_recon_nufft_diff=squeeze(nav_im_recon_nufft_diff);
...
for dyn = 1:nav_dyn
    for y = 1:nav_dimy
        nav_ima_phase_unwrapped(:,y,:,dyn) = GoldsteinUnwrap2D_r1_func(squeeze(nav_im_recon_nufft_diff(:,y,:,dyn)), Manually_seed_point);
        figure(22);
        subplot(121); imshow(squeeze(angle(nav_im_recon_nufft_diff(:,y,:,dyn))),[-pi pi]); title('L: wraped R: unwrapped ') ;  colorbar
        subplot(122); imshow(squeeze(nav_ima_phase_unwrapped(:,y,:,dyn)),[-pi pi]); colormap jet;title(['dyn: ',num2str(dyn),'y: ',num2str(y)]);  colorbar
        drawnow;
...
phase_wrapped = angle(nav_im_recon_nufft_diff);

figure(601);
subplot(121); montage(permute(squeeze(phase_wrapped(:,8,:,:)),[1 2 4 3]),'displayrange',[-pi pi]); colormap jet;
subplot(122); montage(permute(squeeze(nav_ima_phase_unwrapped(:,8,:,:)),[1 2 4 3]),'displayrange',[-pi pi]); colormap jet;
% ===================display wrapped phase difference
phase_test_ch = phase_wrapped(:,8,:,:);
for shot_idx = 1:20
...
phase_test_ch = nav_ima_phase_unwrapped(:,:,dyn,:);
for shot_idx = 1:20
    phase_test_ch_diff(:,:,1,shot_idx) = phase_test_ch(:,:,1,shot_idx) -phase_test_ch(:,:,1,4) ;
end
