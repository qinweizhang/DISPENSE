...
clear; clc; close all
cd('/home/qzhang/lood_storage/divi/Users/qzhang/Data/2018_02_25_SND_brain_kerry')
...
data_fn         = 'di_25022018_1340261_2_2_wip_sc2_3d_snd_brain_4bV4.raw';
sense_ref_fn    = 'di_25022018_1339503_1000_5_wip_senserefscanV4.raw';
coil_survey_fn  = 'di_25022018_1335225_1000_2_wip_coilsurveyscanV4.raw';

trj_mat_fn = 'traj_for_Sc2.mat';

%% Check if sense map channel order matched with data channel order
edit check_channel_order.m
...
for d = 1:length(dyn_recon)
    ...
    if(recon_par.update_SENSE_map)
        ...
        nav_sense_map_perm = nav_sense_map_raw(:,:,:,[1:5 19 22 6:18 20 21 23:30]); % manually detected in this case the sense channel order is not matched with image
        nav_sense_Psi_perm = nav_sense_Psi_raw([1:5 19 22 6:18 20 21 23:30], [1:5 19 22 6:18 20 21 23:30]);
        
        %compress sense map and sense_Psi
        if(exist('Nav_VirtualCoilMartix','var'))
            if(~isempty(Nav_VirtualCoilMartix))
                [nav_sense_map, nav_sense_Psi] = compress_sense_map_Psi(Nav_VirtualCoilMartix, nav_sense_map_perm, nav_sense_Psi_perm);
            end
        end
    end
...
sense_map_temp_reorder = sense_map_temp(:,:,:,[1:5 19 22 6:18 20 21 23:30]); % manually detected in this case the sense channel order is not matched with image
sense_Psi_reorder = TSE.sense_Psi([1:5 19 22 6:18 20 21 23:30], [1:5 19 22 6:18 20 21 23:30]);

%----compress sense map and sense_Psi
if(isfield(TSE, 'VirtualCoilMartix'))
    if(~isempty(TSE.VirtualCoilMartix))
        [sense_map_temp, TSE.sense_Psi] = compress_sense_map_Psi(TSE.VirtualCoilMartix, sense_map_temp_reorder, sense_Psi_reorder);
    end
end
