...
save traj_Sc10_12_13.mat trj_meas_kx trj_meas_ky trj_meas_kz
%% Recon with measured Trajecotry
clear
fn = 'dp_25072017_1801140_9_2_wipdpstisosadlinearV4.raw';
...
%% -----BART recon -------%
% =======  LOAD DATA
clear; close all;  load('traj_Sc10_12_13.mat'); load('data_Sc09_3D.mat')
...
scale_foctor_kx_ky = max([15/max(abs(trj_meas_kx)),15/max(abs(trj_meas_ky))]);
...
reco2 = ((bart('pics -S -r0.001 -t', trj_bart', sig_bart, sens_map)));
...
clear; close all; clc;
fn = 'data_Sc09_3D.mat';
load('traj_Sc10_12_13.mat'); load(fn);
...
data_fn = 'data_Sc09_3D.mat';
trj_fn = 'traj_Sc10_12_13.mat';
...
if(sense_map_recon)
    ...
    if(exist('sens_map'))
        ...
        kspa_TSE_resize = bart('resize -c 0 30 1 30 2 15', kspa_sorted);
...
for shot_nr = 1:1
    ...
    if(sense_map_recon) %all in one recon
        ...
        nav_im_recon_nufft(:,:,:,:,shot_nr)=regularizedReconstruction(A,sig_nufft,'maxit',20);
...
if(sense_map_recon) %all in one recon
    figure(37); montage(permute(abs(nav_im_recon_nufft), [1 2 4 3]),'displayrange',[]); title('all slices rss')
    figure(38); montage(permute(abs(nav_im_recon_nufft), [2 3 4 1]),'displayrange',[]); title('all slices rss')
else
