...
save traj_Sc18_19_20.mat trj_meas_kx trj_meas_ky trj_meas_kz
%% Recon with measured Trajecotry
clear
fn = 'dp_26072017_1234048_4_2_wip_dpsti_sosad_linear_spiralsense_xyV4.raw';
...
%% NUFFT recon.  3D
clear; close all; clc;
load('traj_Sc18_19_20.mat'); load('data_Sc04_3D.mat')
%scle trajectory to match kx_range, ky_range (-pi to pi)
ignore_kz = 0;
recon_dim  = [24 24 12];
...
if(sense_map_recon)
    ...
    che=create_checkerboard([size(kspa_TSE_resize,1),size(kspa_TSE_resize,2),size(kspa_TSE_resize,3)]);
    kspa_TSE_resize=bsxfun(@times,kspa_TSE_resize,che);
    sens=bart('ecalib -S -m1',kspa_TSE_resize);
    sens_map =  sens;
...
% for shot_nr = 1:1
    ...
    if(sense_map_recon) %all in one recon
        ...
        nav_im_recon_nufft(:,:,:,:,shot_nr)=regularizedReconstruction(A,sig_nufft,'maxit',10);
