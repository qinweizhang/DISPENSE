...
subplot(M,N,2);
doplot(t,k,'k',pls,acq);
xlabel('time(ms)');
ylabel('K-space (cm^{-1})');
title('K-space vs time');

subplot(M,N,3);
doplot(t,g*G_cm2mT_m,'g',pls,acq);
xlabel('time(ms)');
ylabel('Gradient (mT/m)');
title('Gradient vs time');

subplot(M,N,4);
doplot(t,s*G_cm_s2mT_m_ms,'slew',pls,acq);
xlabel('time(ms)');
