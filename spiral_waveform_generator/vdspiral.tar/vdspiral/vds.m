...
disp(['Oversampling rate is: ', num2str(oversamp)]);
...
subplot(2,2,3);
plot(time,real(g)*G_cm2mT_m,'r--',time,imag(g)*G_cm2mT_m,'b--',time,abs(g)*G_cm2mT_m,'k-');
title('g vs t');
ylabel('G (mT/m)');

subplot(2,2,4);
plot(time,real(s)*G_cm_s2mT_m_ms,'r--',time,imag(s)*G_cm_s2mT_m_ms,'b--',time,abs(s)*G_cm_s2mT_m_ms,'k-');
title('s vs t');
